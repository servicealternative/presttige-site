...
def lambda_handler(event, context):
    try:
        ...
        item = db.get("Item", {})
        referral_code = (item.get("referral_code") or "unknown").strip() or "unknown"
        if "Item" not in db:
            return response(404, {"error": "lead_not_found"})

        created_at_raw = (item.get("created_at") or "").strip()
        ...
        if product == "founder":
            ...
            metadata["product_type"] = "founder"

        elif product == "access":
            price = ACCESS_PRICE_ID
            metadata["product"] = "access"
            metadata["product_type"] = "access"
